...
x_fw = LSTM(output_dim=100, input_dim=128, return_sequences=True,W_regularizer=l2(0.01), consume_less='mem')(inputs)

x_bw = LSTM(output_dim=100, input_dim=128, return_sequences=True,W_regularizer=l2(0.01), consume_less='mem',go_backwards=True)(inputs)

vec_fw = td_lstm(x_fw,indexes_fw)
vec_bw = td_lstm(x_bw,indexes_bw)
...
##########################################
    ...
    for n in range(5):
        ...
        for epoch in range(50):
            ...
            #train data padding and cropping
            X_train1=[]
            y_train1=[]
            indexes_f_train1=[]
            indexes_b_train1=[]
            ...
            indexes_f_test1=np.asarray(indexes_f_test1)
            indexes_b_test1=np.asarray(indexes_b_test1)
            ...
            for i in range(num_minibatch):
                ...
                sess.run(train_step, feed_dict={inputs: X, indexes_fw:forward, indexes_bw:backward,  labels: Y, K.learning_phase(): 1})    
                
                

    
            # testing accuracy
            test_acc= acc_value.eval(feed_dict={inputs: X_test1, indexes_fw:indexes_f_test1, indexes_bw:indexes_b_test1, labels: y_test1,K.learning_phase(): 0})
            pre=predictions.eval(feed_dict={inputs: X_test1, indexes_fw:indexes_f_test1, indexes_bw:indexes_b_test1, labels: y_test1,K.learning_phase(): 0})
