...
aspect_id = Input(dtype=np.float32,batch_shape=[inputs_shape[0],inputs_shape[1],6])# where the predefined nb of aspects in a sentence is 6, should be changed according to dataset.

x_batch_dot = K.batch_dot(lstm1,aspect_id, axes=[1, 1])
...
# aspect from 1 to 8
aspect1=x_batch_dot[:,:,0]
aspect2=x_batch_dot[:,:,1]
aspect3=x_batch_dot[:,:,2]



#lstm input

seq_input= tf.concat(1,[tf.expand_dims(aspect1, 1),tf.expand_dims(aspect2, 1)])
seq_input1=tf.concat(1,[seq_input,tf.expand_dims(aspect3, 1)])



var0= LSTM(output_dim=256,  input_length=3,activation='relu',return_sequences=False,W_regularizer=l2(0.01), inner_activation='sigmoid')(seq_input1)
